...
const MAIDBOT_CONFIG = JSON.parse(fs_1.default.readFileSync('./src/config.json', 'utf8'));
const BOTNAME = `+${MAIDBOT_CONFIG.name}`;
const BOTPREFIX = MAIDBOT_CONFIG.prefixes[0];
...
let shooer = '';
const MAX_GREETS = 8;
let usersToGreet = [...salutations_js_1.default.usersToGreet];
let specialGreets = true;
let msgRot = rand(0, MAX_GREETS);
let oldMsgRot = msgRot;
const votesFilename = './src/votes.json';
let votesJSON = JSON.parse(fs_1.default.readFileSync(votesFilename, 'utf8'));
...
function reset() {
    active = true;
    shooer = '';
    usersToGreet = salutations_js_1.default.usersToGreet.slice(0);
    specialGreets = true;
    votesJSON = JSON.parse(fs_1.default.readFileSync(votesFilename, 'utf8'));
    votes = votesJSON.votes;
}
function test(msg) {
    if (msg.author.id != BOSS_ID) {
        return;
    }
}
...
function msgTxtFilter(keywords) { return keywords ? keywords.replace(/\W+/gi, '+') : ""; }
// Type guards
function isTextCh(arg) { return arg && arg.type && arg.type === "text"; }
function isDMCh(arg) { return arg && arg.type && arg.type === "dm"; }
function rand(min, max) { return Math.floor(Math.random() * (max - min + 1)) + min; }
function startsWithAny(s, a) { return a.some((e) => s.startsWith(e)); }
...
async function req(url) {
    let html = "";
    try {
        html = await r2_1.default(url).text;
    }
    catch (err) {
...
CLIENT.on('guildMemberAdd', member => {
    ...
    else if (((arg) => arg)(member) && member.bot) {
        console.log('\tand it is a bot!');
    }
    ...
    if (channel && isTextCh(channel)) {
        channel.messages.fetch({ limit: 1 }).then(messages => {
            ...
            for (let i = 0; msgRot == oldMsgRot && i < MAX_GREETS; i++) {
                msgRot = rand(0, MAX_GREETS);
            } // Few attempts to avoid repeating same greeting
            oldMsgRot = msgRot;
...
CLIENT.on('message', async (message) => {
    ...
    const msgUserId = message.author.id;
    ...
    if (message.author.id === BOSS_ID) {
        ...
        if (specialGreets && msg === `${BOTNAME} SHUSH`) {
            specialGreets = false;
        }
    }
    ...
    if (!active) {
        ...
        if (args[3] === 'FOR') {
            if (args[4].startsWith('<@!')) {
                shooer = `<@${args[4].slice(3)}`;
            }
            else if (args[4].startsWith('<@')) {
                shooer = args[4];
            }
            else if (args[4] === 'ME') {
                shooer = `<@${message.author.id}>`;
            }
        }
        active = false;
        if (shooer !== '') {
    ...
    if (specialGreets && usersToGreet.includes(message.author.id)) {
        const greet = salutations_js_1.default.greet(message.author.id);
        if (greet) {
    ...
    if (message.author.id !== BOSS_ID && args.includes(`<@${BOSS_ID}>`)) {
        message.channel.send('Manager has been called!');
    }
    if (args[0] === 'PING') {
        ...
        if (entry === 'COUNT') {
            ...
            fs_1.default.writeFile(votesFilename, JSON.stringify(votesJSON), "utf8", err => {
                if (err) {
            ...
            for (let entrant of entrants) {
                ...
                if (!votes[entrant]) {
                    votes[entrant] = new Array(message.author.tag);
                }
                else {
                    votes[entrant].push(message.author.tag);
                }
            }
        ...
        if (/(?<=<[@]!?)\d+(?=>)/.test(args[3])) {
            ...
            rickRoll(message.channel).catch(logError);
        } // Otherwise just send to received command's channel
    }
        ...
        message.channel.send(salutations_js_1.default.welcome(rand(0, MAX_GREETS), args.slice(1).join(", ")));
    }
        ...
        if (args[1] && args[0].endsWith('MSG')) {
            ...
            if (args[2] && /<#([0-9]+)>/.test(args[2])) {
                ...
                const targetChannelID = args[2].match(/<#([0-9]+)>/)[1];
                let targetChannelTmp = CLIENT.channels.cache.get(targetChannelID);
                if (isTextCh(targetChannelTmp)) {
                    targetChannel = targetChannelTmp;
                }
            }
        ...
        if (args[1].startsWith('"')) {
            ...
            if (/"(.*?)"/.test(what)) {
                ...
                if (where) {
                    ...
                    CLIENT.channels.cache.get(where[0]).send(text)
